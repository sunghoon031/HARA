function [R_est, time_initialization, time_optimization] = ...
    RunHARA_usingNumberOfInlierMatches(...
    nViews,...
    edge_IDs,...
    RR,...
    nInliers_for_all_pairs,...
    nSamples,...
    s_init,...
    thrs_proportion,...
    triplet_thr,...
    ...
    % - nInliers_for_all_pairs = Number of valid 2D-2D correspondences
    %                            (i.e., inlier feature matches) between a
    %                            pair of views. 
    %                            The number of inliers between view i and j
    %                            is given by nInliers_for_all_pairs(i,j).
    ...
    %                this check. 
    %                (default = 1.0)
    % - nInliers_thrs = Corresponds to d_1, d_2, ... in the paper.
    %                   If the number of inlier matches for a given edge is
    %                   below this threshold, we ignore it during the
    %                   initialization.
    %                   (default = [5, 0])
    %
    % Output:
    ...
    inlier_idx = 1;
    nInliers_thr = nInliers_thrs(inlier_idx);
    ...
    while(min(isFamily)==0)
        ...
        while(~isempty(newFamily))
            ...
            neighbors_ = edge_info{id_to_check};
            neighbors = [];
            for j = neighbors_
                if (nInliers_for_all_pairs(j, id_to_check) >= nInliers_thr)
                    neighbors(end+1) = j;
                end
            end
            ...
            for jj = 1:length(neighbors)-1
                ...
                for kk = jj + 1:length(neighbors)
                    k = neighbors(kk);
                    if (A(j,k)==0 || nInliers_for_all_pairs(j, k) < nInliers_thr)
                        continue;
                    end
        ...
        for i = voters
            for j = edge_info{i}
                if (~isFamily(j) && nInliers_for_all_pairs(j, i) >= nInliers_thr)
                    votes(j) = votes(j) + 1;
                end
            end
        end
        ...
        if (maxVotes == 0)
            if (inlier_idx == length(nInliers_thrs))
                break;
            end

            % Switch to a smaller nInliers_thr:
        ...
        for j = edge_info{id_to_check}
            if (isFamily(j) && nInliers_for_all_pairs(j, id_to_check) >= nInliers_thr)
                c = c + 1;
